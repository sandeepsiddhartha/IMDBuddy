...
# Copy Chrome manifest (generated by generate-manifests.sh)
# The manifest should have been generated directly to chrome-extension/ by generate-manifests.sh
# Since chrome-extension is a symlink to dist/chrome-extension, the file is already in place
if [ -f "chrome-extension/manifest.json" ]; then
    print_success "Chrome manifest is already in place via symlink"
else
    print_error "No Chrome manifest found! Run ./build-tools/generate-manifests.sh first"
    exit 1
fi
